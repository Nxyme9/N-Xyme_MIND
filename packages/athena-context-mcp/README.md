...
A Python MCP server that provides context injection from Athena's memory bank into OpenCode sessions. Reads identity, user, active context, and constraints defined in `athena.yaml`.
...
```bash
python -m athena_context_mcp
```

### SSE mode (for remote access)
```bash
python -m athena_context_mcp --sse --port 8766
```
...
Override via `ATHENA_CONTEXT_ROOT` environment variable.