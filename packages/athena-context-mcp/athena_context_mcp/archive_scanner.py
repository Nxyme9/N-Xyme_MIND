...
class ArchiveScanner:
    ...
    def __init__(
        ...
        logger.info(f"ArchiveScanner: Initialized (db={self.db_path}, sisyphus={self.sisyphus_dir})")
                        ...
                        if line.strip():
                            ...
                            if entry.get("task_id") == session_id:
                                ...
                                if entry.get("description"):
                                    context["tasks"].append(entry.get("description", ""))
                                context["success"] = entry.get("success", True)
                                
                                # Extract operations from metadata
                                metadata = entry.get("metadata", {})
                                if isinstance(metadata, dict):
                                    context["operations"].extend(metadata.get("operations", []))
            except Exception as e:
                logger.error(f"Failed to read session-log.jsonl: {e}")
        ...
        if context["found"]:
            try:
                ...
                if rows:
                    ...
                    context["success_rate"] = sum(1 for r in rows if r[2]) / len(rows) if rows else 0
