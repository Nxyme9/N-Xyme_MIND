...
def store(content: str, kind: str = "memory", scope: str = "session", metadata: dict = None):
    ...
    try:
        ...
        return create_memory(content, kind=kind, scope=scope, tags=[], metadata=metadata or {})
...
def recall_session(session_id: str = None, limit: int = 50):
    ...
    """
    try:
        from packages.memory_core.mcp_server import recall_session as mcp_recall
