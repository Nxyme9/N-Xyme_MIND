...
- MCP servers (memory_core, learning_engine)
...
class HealthCheck:
    ...
    def check_mcp_servers(self) -> HealthStatus:
        ...
        try:
            # Check memory_core MCP server module
            try:
                from packages.memory_core import mcp_server

                details["memory_core_mcp"] = "importable"
            except ImportError as e:
                details["memory_core_mcp"] = f"import_error: {str(e)}"
            except Exception as e:
                details["memory_core_mcp"] = f"error: {str(e)}"
    ...
    def check_cognitive_engines(self) -> HealthStatus:
        ...
        try:
            ...
            try:
                from packages.memory_core.cognitive.forgetting import AdaptiveDecay

                db_path = Path(self.DB_PATH)
                forgetting = AdaptiveDecay(db_path)
                stats = forgetting.get_decay_stats()
                details["forgetting"] = f"ok (stats available)"
            except ImportError as e:
            ...
            try:
                ...
                details["trust"] = f"ok (stats available)"
            except ImportError as e:
            ...
            try:
                ...
                details["priority"] = f"ok (stats available)"
            except ImportError as e:
