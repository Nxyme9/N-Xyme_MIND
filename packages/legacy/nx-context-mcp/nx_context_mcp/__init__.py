...
# ---------------------------------------------------------------------------
# Anthropic-Sourced Modules (from leaked source code)
# ---------------------------------------------------------------------------
# These modules implement bleeding-edge patterns from Anthropic's internal source

try:
    from nx_context_mcp import auto_compact
    from nx_context_mcp import tool_result_storage
    from nx_context_mcp import tool_orchestrator
    from nx_context_mcp import subagent_manager
    from nx_context_mcp import token_counter
    from nx_context_mcp import caching
    from nx_context_mcp import tool_factory
    from nx_context_mcp import tool_hooks

    _ANTHROPIC_MODULES_LOADED = True
except ImportError as e:
    logger.warning(f"Some Anthropic modules not loaded: {e}")
    _ANTHROPIC_MODULES_LOADED = False
...
mcp = FastMCP(
    ...
    version="1.0.0",
    instructions=(
        "N-Xyme Context MCP Server — inject context from Athena memory bank.\n\n"
        "Tools:\n"
...
def get_project_root() -> Path:
    ...
    if "NX_CONTEXT_ROOT" in os.environ:
        return Path(os.environ["NX_CONTEXT_ROOT"])
...
def update_context_timestamp(filename: str) -> dict:
    """Update timestamp in context file frontmatter when accessed."""
    memory_bank = get_memory_bank_path()
    file_path = memory_bank / filename
    ...
    try:
        ...
        if content.startswith("---"):
            ...
            if len(parts) >= 3:
                ...
                fm = yaml.safe_load(frontmatter) or {}
                fm["last_accessed"] = timestamp
                fm["accessed_count"] = fm.get("accessed_count", 0) + 1

                # Rebuild file with updated frontmatter
                new_frontmatter = yaml.dump(fm, default_flow_style=False)
                new_content = f"---\n{new_frontmatter}---\n{body}"
                file_path.write_text(new_content, encoding="utf-8")

                return {"updated": True, "timestamp": timestamp}
...
def get_active_context() -> dict:
    ...
    """
    # Auto-refresh timestamp on access
    update_context_timestamp("activeContext.md")
...
def get_style_context() -> dict:
    ...
    """
    try:
        from nx_context_mcp.style_learner import get_learner
...
def get_archive_context(query: str = "", max_sessions: int = 3) -> dict:
    """
    ...
    """
    try:
        from nx_context_mcp.archive_scanner import get_scanner
        ...
        summary = scanner.build_context_summary(
            queries=[query], max_sessions_per_query=max_sessions
        )
    ...
    try:
        ...
        for item in sorted(agents_dir.iterdir()):
            if item.is_file() and item.suffix in [".md", ".yaml", ".yml", ".json"]:
                ...
                agent_info = {"name": item.name, "path": str(item), "type": "directory"}
                result["agents"].append(agent_info)
    except Exception as e:
        result["error"] = str(e)
        ...
        for item in sorted(workflows_dir.iterdir()):
            if item.is_dir():
                ...
                result["workflows"]["root"].append(
                    {"name": item.stem, "path": str(item), "type": item.suffix[1:]}
                )
...
def inject_context(context_type: str = "active", output_path: str = None) -> dict:
    """
    ...
    # Build context block
    context_block = []
    context_block.append("# Athena Context Injection")
    ...
    if context_type in ["all", "user"]:
        try:
            ...
            if style:
                ...
                context_block.append(
                    f"- Communication: {style.get('communication_style', 'unknown')}"
                )
                context_block.append(
                    f"- Directness: {style.get('communication_directness', 'unknown')}"
                )
                context_block.append(
                    f"- Preferred agents: {style.get('preferred_agents', [])}"
                )
        ...
        try:
            ...
            if recent.get("sessions"):
                ...
                for sess in recent["sessions"][:3]:
                    context_block.append(
                        f"- {sess.get('session_id', 'unknown')}: {sess.get('summary', 'No summary')}"
                    )
                context_block.append("")
...
def get_capabilities() -> dict:
    ...
    Returns:
        dict with available tools and their purposes.
    """
    return {
        "server": "nx-context",
        "version": "1.0.0",
        "tools": [
            {
                ...
                "desc": "Current active context from memory bank",
            },
            {"name": "get_product_context", "desc": "Product/identity context (soul)"},
            {"name": "get_user_context", "desc": "User preferences and context"},
            {"name": "get_constraints", "desc": "Behavioral constraints and rules"},
            {
                "name": "get_user_profile",
                "desc": "Immutable user identity (psychological profile)",
            },
            {
                "name": "get_style_context",
                "desc": "Personalized style from usage patterns",
            },
            {"name": "get_archive_context", "desc": "Relevant past session context"},
            {"name": "get_bmad_agents", "desc": "List available BMAD agents"},
            {"name": "get_bmad_workflows", "desc": "List BMAD workflows by phase"},
            {
                "name": "inject_context",
                "desc": "Write context into session for prompt injection",
            },
...
def health_check() -> dict:
    ...
    memory_bank = project_root / ".context" / "memory_bank"
    if memory_bank.exists():
        checks["memory_bank"] = "ok"
        files = list(memory_bank.glob("*.md"))
        checks["files_count"] = len(files)
    else:
        checks["memory_bank"] = "missing"
    ...
    if bmad_dir.exists():
        ...
        checks["bmad"] = "not_found"

    return {
        "status": "healthy" if "memory_bank" in checks else "degraded",
        "checks": checks,
        "mcp": "nx-context",
    }
