...
class StyleLearner:
    ...
    def _init_db(self) -> None:
        ...
        try:
            ...
            # Create indexes
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_style_task_types_timestamp ON style_task_types(timestamp)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_style_delegations_agent ON style_delegations(agent)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_style_delegations_timestamp ON style_delegations(timestamp)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_style_communication_timestamp ON style_communication(timestamp)"
            )
    ...
    def _load_data(self) -> None:
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Loaded style data: {len(self._task_type_counts)} task types, {len(self._agent_counts)} agents"
            )
        except Exception as e:
    ...
    def _update_profile(self) -> None:
        ...
        if self._task_type_counts:
            ...
            preferred_task_types = [
                t[0] for t in top_tasks if t[1] >= self.min_occurrences
            ]
        else:
            preferred_task_types = []
        ...
        if self._agent_counts:
            ...
            preferred_agents = [
                a[0] for a in top_agents if a[1] >= self.min_occurrences
            ]
        else:
            preferred_agents = []
        ...
        success_rate = (
            success_count / len(self._delegation_history)
        ...
        if self._delegation_history:
            avg_latency = sum(d.latency_ms for d in self._delegation_history) / len(
    ...
    def get_style_context(self) -> Dict[str, Any]:
        ...
        context = {
            "style_profile": {
                ...
                "communication_directness": profile.get(
    ...
    def get_task_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_task_type": self._task_type_counts.most_common(1)[0][0]
            if self._task_type_counts
            else None,
    ...
    def get_delegation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_agent": self._agent_counts.most_common(1)[0][0]
            if self._agent_counts
            else None,
            "success_rates": {k: round(v, 2) for k, v in success_rates.items()},
            "avg_latency_ms": round(
                sum(d.latency_ms for d in self._delegation_history)
    ...
    def get_style_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_style": self._profile.get("communication_style", "unknown")
            if self._profile
            else "unknown",
            "current_directness": self._profile.get(
                "communication_directness", "unknown"
            )
            if self._profile
            else "unknown",
        }
