...
# Memory Core imports
try:
    from packages.memory_store import search as memory_search, stats as memory_stats
    from packages.memory_store import recall_session, store as memory_store
    from packages.memory_store.retrievers import TEMPRRetriever
...
class ChatBackend:
    ...
    async def get_llm_response(
        self,
        messages: list[dict],
        model: str = "qwen2.5-coder-7b-q4_k_m",
        system_prompt: str = "",
    ) -> dict:
    ...
    async def build_context(self, query: str, intents: list[str]) -> dict:
        ...
        if "health" in intents or "stats" in intents:
            ...
            if health.get("status") == "ok":
                context_parts.append("## System Health (L0)")
                context_parts.append(
        ...
        if "command" in intents:
            ...
            if cmd_result.get("status") == "ok":
                context_parts.append("## Command Output")
                context_parts.append(cmd_result.get("output", "Success")[:500])
            else:
                context_parts.append("## Command Error")
        ...
        if "logs" in intents:
            ...
            if logs_result.get("status") == "ok":
                ...
                context_parts.append("## Logs Error")
            ...
            for alias_name in self._init_aliases().keys():
                if f"do {alias_name}" in query_lower:
                    ...
                    if alias_result.get("status") == "ok":
                        ...
                        context_parts.append("## Alias Error")
    ...
    async def execute_mcp_tool(self, tool_name: str, arguments: dict = None) -> dict:
        ...
        MCP_TOOL_MAP = {
            "search_memory": {
                "module": "packages.memory_store.mcp_server",
