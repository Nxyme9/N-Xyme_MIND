...
def get_learning_config():
    try:
        from packages.memory_store.learning_config import get_config as _gc
...
def save_learning_config(updates: dict) -> bool:
    try:
        from packages.memory_store.learning_config import get_config, save_config
...
def reset_learning_config() -> bool:
    try:
        from packages.memory_store.learning_config import reset_config
...
def get_drive_status():
    try:
        from packages.memory_store.config import health_check_drives, list_drives
...
def get_system_stats():
    ...
    try:
        ...
        pass  # Ollama/llama-server check failed
    try:
    ...
    try:
        from packages.memory_store.indexing.embedder import get_indexed_count as _gc
...
class SettingsScreen(ModalScreen[None]):
    ...
    def _build_stats_widgets(self):
        ...
        table.add_row(
            "GGUF Server",
            "Running" if stats.get("llama_server_running") else "Stopped",
