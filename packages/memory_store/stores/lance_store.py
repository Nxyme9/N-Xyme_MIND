...
from packages.memory_store.stores.base import SearchResult, VectorStore as VectorStoreABC
...
class LanceDBVectorStore(VectorStoreABC):
    ...
    def purge_deleted(self) -> int:
        ...
        try:
            ...
            # Batch delete - more efficient than one-by-one
            # LanceDB delete() can take multiple IDs via OR expression
            if len(deleted_ids) > 0:
                # Build batch delete expression
                id_expressions = [f"id = '{rid}'" for rid in deleted_ids]
                where_clause = " OR ".join(id_expressions)

                try:
                    # Try batch delete first (faster)
                    self._table.delete(where_clause)
                    logger.info(f"Purged {len(deleted_ids)} deleted records (batch)")
                except Exception as batch_error:
                    # Fallback to individual deletes if batch fails
                    logger.warning(
                        f"Batch purge failed, using individual deletes: {batch_error}"
                    )
                    for record_id in deleted_ids:
                        try:
                            ...
                            logger.warning(
                                f"Failed to delete {record_id}: {individual_error}"
                            )
                            continue

            # Verify purge by checking no deleted records remain
            verify_df = self._table.to_pandas()
            remaining_deleted = verify_df[verify_df["deleted"] == True]["id"].tolist()
            if remaining_deleted:
                logger.warning(
                    f"Verification: {len(remaining_deleted)} records still marked deleted after purge"
                )
...
_lance_store_instance: Optional[LanceDBVectorStore] = None
_lance_store_lock = None  # Will be initialized on first use
...
def get_lance_store(
    ...
    """Get a LanceDB vector store instance with connection pooling.

    Uses singleton pattern to reuse the same instance across calls,
    avoiding the overhead of creating new connections each time.
    ...
    Returns:
        LanceDBVectorStore instance (singleton)
    """
    global _lance_store_instance, _lance_store_lock

    # Lazy initialization of lock
    if _lance_store_lock is None:
        import threading

        _lance_store_lock = threading.Lock()

    # Fast path - instance already exists
    if _lance_store_instance is not None:
        return _lance_store_instance

    # Slow path - need to create instance
    with _lance_store_lock:
        # Double-check after acquiring lock
        if _lance_store_instance is None:
            config = LanceDBConfig(path=path, index_type=index_type)
            _lance_store_instance = LanceDBVectorStore(config=config)
            logger.info("LanceDB connection pool: initialized singleton instance")

    return _lance_store_instance


def reset_lance_store() -> None:
    """Reset the singleton instance (for testing or recovery).

    Call this to force recreation of the LanceDB connection on next request.
    """
    global _lance_store_instance
    if _lance_store_instance is not None:
        logger.info("LanceDB connection pool: resetting singleton instance")
        # Close the database connection
        try:
            if _lance_store_instance._db is not None:
                _lance_store_instance._db.close()
        except Exception as e:
            logger.warning(f"Error closing LanceDB connection: {e}")
        _lance_store_instance = None
