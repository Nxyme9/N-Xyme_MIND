    ...
    def check_embedding_coverage(self) -> dict:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Failed to get ChromaDB count: {e}")
                chroma_count = 0
    ...
    def check_qlearning_state(self) -> dict:
        ...
        try:
            ...
            if routing_db.exists():
                ...
                cursor = conn.execute("SELECT COUNT(*) FROM delegation_outcomes")
...
def main():
    ...
    for check_name, check_result in result.get("checks", {}).items():
        logger.info(f"  {check_name}: {check_result.get('status')}")
