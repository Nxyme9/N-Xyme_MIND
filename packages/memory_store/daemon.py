        ...
        while not self._learning_stop_event.is_set():
            try:
                ...
                try:
                    ...
                    # Helper function to measure GGUF llama-server health (zero network overhead)
                    def measure_ollama_health() -> tuple[float, float]:
                        """Measure GGUF llama-server response time and error rate."""
                        import urllib.request

                        start = time.time()
                        try:
                            # Primary: Check GGUF llama-server on port 8080
                        ...
                        # Fallback: Check Ollama on port 11434 (deprecated)
                        try:
                            req = urllib.request.Request(
