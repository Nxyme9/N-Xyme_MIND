"""Memory Store Configuration — Centralized configuration for memory_store package.
...
Usage:
    from memory_store.config import get_config, MemoryStoreConfig
...
@dataclass
class Config:
    """Main memory_store configuration container."""
...
def get_config(force_reload: bool = False) -> Config:
    """Get the current memory_store configuration.

    Args:
        force_reload: If True, bypass cache and reload from sources.

    Returns:
        Config dataclass with all memory_store settings.
    ...
    logger.debug(
        f"Loaded memory_store config: DB={_config_cache.database.path}, RRF_k={_config_cache.retrieval.rrf_k}"
    )
...
def update_config(updates: dict) -> Config:
    ...
    Example:
        >>> from memory_store.config import update_config
...
def validate_config(config: Optional[Config] = None) -> tuple[bool, list[str]]:
    ...
    if config.vector_store.index_type not in ("hnsw", "flat", "ivf"):
        errors.append("vector_store.index_type must be one of: hnsw, flat, ivf")
...
def save_config(config: Optional[Config] = None, path: Optional[Path] = None) -> None:
    ...
    logger.info(f"Saved memory_store config to {path}")
...
if __name__ == "__main__":
    config = get_config()
    logger.info("Memory Core Configuration")
    logger.info("=" * 50)
    logger.info(f"Database path: {config.database.path}")
    logger.info(f"  Pool size: {config.database.pool_size}")
    logger.info(f"  Timeout: {config.database.connection_timeout_ms}ms")
    logger.info("")
    logger.info("Retrieval:")
    logger.info(f"  RRF k: {config.retrieval.rrf_k}")
    logger.info(f"  Trust weight: {config.retrieval.trust_weight}")
    logger.info(f"  MMR lambda: {config.retrieval.mmr_lambda}")
    logger.info(f"  Default top-k: {config.retrieval.default_top_k}")
    logger.info("")
    logger.info("Cognitive:")
    logger.info(f"  Decay enabled: {config.cognitive.decay_enabled}")
    logger.info(f"  Trust initial: {config.cognitive.trust_initial}")
    logger.info(f"  Similarity threshold: {config.cognitive.similarity_threshold}")
    logger.info("")
    ...
    if env_vars:
        ...
        for var in env_vars[:10]:
            logger.warning(f"  {var}={os.environ[var]}")
        if len(env_vars) > 10:
            logger.warning(f"  ... and {len(env_vars) - 10} more")
