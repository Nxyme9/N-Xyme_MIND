...
TRANSCRIPTS_ROOT = os.environ.get(
    "TRANSCRIPTS_ROOT", "/mnt/Library/nx_openmore/context/claude"
)
...
def migrate_transcript(
    fname: str, max_content_size: int = 5000
...
def migrate_all(
    batch_size: int = 50, limit: int = 200
    ...
    for idx, fname in enumerate(transcripts[:limit]):
        logger.info(
            f"Migrating transcript {idx + 1}/{min(len(transcripts), limit)}: {fname}"
        )
...
if __name__ == "__main__":
    logger.info("Transcript migration module ready")
    logger.info(f"Transcripts path: {TRANSCRIPTS_PATH}")
    logger.info(f"Accessible: {check_transcripts_accessible()}")
    logger.info(f"Found: {count_transcripts()} transcripts")
