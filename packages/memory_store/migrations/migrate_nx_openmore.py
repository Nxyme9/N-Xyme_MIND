...
def validate_table_name(table: str) -> bool:
    ...
    if not table.replace("_", "").isalnum() or len(table) > 64:
        return False
    ...
    try:
        ...
        for table in tables:
            ...
            if not validate_table_name(table):
                logger.warning(f"Skipping table {table} - not in whitelist")
                continue
            try:
                # Use parameterized LIMIT - only table name uses f-string (whitelist validated)
    ...
    try:
        ...
        for table in table_names:
            ...
            if not validate_table_name(table):
                ...
                continue
            try:
                # Use whitelist-validated table name
...
def check_sources() -> Dict[str, bool]:
    ...
    return {name: os.path.exists(path) for name, path in SOURCES.items()}
...
if __name__ == "__main__":
    # Test imports
    logger.info("nx_openmore migration module ready")
