...
class TrustAwareRetrieval:
    ...
    def _ensure_table(self) -> None:
        ...
        try:
            conn.execute("""CREATE TABLE IF NOT EXISTS memory_trust (
    ...
    def decay_unverified(self, days: int = 1) -> dict[str, TrustScore]:
        ...
        for memory_id, score in self.trust_scores.items():
            if score.verification_count == 0:
                score.trust = max(TRUST_MIN, score.trust - (TRUST_UNVERIFIED_DECAY * days))
                score.last_updated = datetime.now(timezone.utc).isoformat()
                self._save_trust_score(score)
    ...
    def get_trust_score(self, memory_id: str) -> Optional[TrustScore]:
    ...
    def _save_trust_score(self, score: TrustScore) -> None:
        ...
        conn = sqlite3.connect(str(self.db_path))
        try:
            conn.execute("""INSERT OR REPLACE INTO memory_trust
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        verified = sum(1 for s in self.trust_scores.values() if s.verification_count > 0)
        contradicted = sum(1 for s in self.trust_scores.values() if s.contradiction_count > 0)
...
def trust_weighted_search(
    ...
    """Convenience function for trust-weighted search."""
    return _trust.get_trust_aware_results(search_results, trust_weight, relevance_weight)