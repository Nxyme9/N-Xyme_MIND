...
class ModelFallback:
    ...
    def _load_local_router(self):
        ...
        module_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "local-router.py")
    ...
    def _try_local_first(self, prompt: str) -> Optional[dict]:
        ...
        if not self.local_router.is_local_available():
            logger.info("Local Ollama not available, skipping local-first routing")
            return None

        complexity = self.local_router.classify(prompt)
        if complexity not in ("simple", "medium"):
            logger.info(f"Task classified as '{complexity}', skipping local-first routing")
            return None

        logger.info(f"Task classified as '{complexity}', trying local model first")
        local_models = self.local_router.get_local_models()
        for local_model in local_models:
            model_key = f"ollama/{local_model}"
            ...
            try:
                ...
                logger.warning(f"Local model {local_model} failed ({error_type}): {str(e)}")
