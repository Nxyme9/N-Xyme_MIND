...
including Q-Learning, reward weights, routing, outcome logging, and feature flags.

Supported override sources (in order of precedence):
1. Environment variables (prefixed with LEARNING_ or FF_)
2. YAML config files (config/feature_flags.yaml, config/kv_cache_config.yaml)
3. JSON config file (path via LEARNING_CONFIG_PATH or default)
4. Hardcoded defaults

Usage:
    from learning_engine.config import get_config, FeatureFlags

    config = get_config()
    print(f"Q-Learning alpha: {config.q_learning.alpha}")

    if FeatureFlags.is_enabled("multi_model_routing"):
...
def _apply_env_overrides(config: dict, prefix: str = "LEARNING_") -> dict:
    ...
    env_mappings = {
        ...
        "SKILL_MAX_FAILURE_RATE": (
            "skill_lifecycle",
            "max_failure_rate_for_acceptable",
        ),
...
def get_config(force_reload: bool = False) -> Config:
    ...
    logger.debug(
        f"Loaded learning_engine config: Q(alpha={_config_cache.q_learning.alpha}, "
...
def validate_config(config: Optional[Config] = None) -> tuple[bool, list[str]]:
    ...
    if not 0.0 <= config.routing.min_success_rate_for_recommendation <= 1.0:
        errors.append(
            "routing.min_success_rate_for_recommendation must be between 0.0 and 1.0"
        )
    ...
    if not 0.0 <= config.skill_lifecycle.min_success_rate_for_good <= 1.0:
        errors.append(
            "skill_lifecycle.min_success_rate_for_good must be between 0.0 and 1.0"
        )
    ...
    if not 0.0 <= config.skill_lifecycle.max_failure_rate_for_acceptable <= 1.0:
        errors.append(
            "skill_lifecycle.max_failure_rate_for_acceptable must be between 0.0 and 1.0"
        )
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print(
        f"Q-Learning: alpha={config.q_learning.alpha}, gamma={config.q_learning.gamma}, "
    ...
    print(
        f"  Quality bonus: threshold={config.reward.quality_bonus_threshold}, "
    ...
    # Feature flags
    print("\nFeature Flags:")
    for f in FeatureFlags.list_enabled():
        print(f"  ✓ {f}")
    if not FeatureFlags.list_enabled():
        print("  (none enabled)")
...
class FeatureFlags:
    """Feature flag management system for ML experiments."""
    ...
    def is_enabled(cls, feature: str) -> bool:
        ...
        enabled = cls._config.get("enabled_features", {}).get(feature, False)

        # Environment variable override (FF_FEATURE_NAME=1)
        env_override = os.environ.get(f"FF_{feature.upper()}")
    ...
    @classmethod
    def get_rollout(cls, feature: str) -> int:
        """Get rollout percentage for feature (0-100)."""
        cls._load()

        if not cls.is_enabled(feature):
            return 0

        return cls._config.get("rollout", {}).get("default_percentage", 10)

    @classmethod
    def in_experiment_group(cls, group: str) -> bool:
        """Check if feature group is active."""
        cls._load()

        features = cls._config.get("experiment_groups", {}).get(group, [])
        return any(cls.is_enabled(f) for f in features)

    @classmethod
    def list_enabled(cls) -> list:
        """List all enabled features."""
        cls._load()

        return [f for f, v in cls._config.get("enabled_features", {}).items() if v]
...
def get_rollout(feature: str) -> int:
    """Get rollout percentage."""
    return FeatureFlags.get_rollout(feature)


def in_group(group: str) -> bool:
    """Check if in experiment group."""
    return FeatureFlags.in_experiment_group(group)
