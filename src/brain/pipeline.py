...
class BrainPipeline:
    def __init__(self):
        ...
        self.breakers = {
            "hephaestus": CircuitBreaker("hephaestus", failure_threshold=3, reset_timeout=60),
            "sisyphus junior": CircuitBreaker("sisyphus junior", failure_threshold=3, reset_timeout=60),
            "oracle": CircuitBreaker("oracle", failure_threshold=3, reset_timeout=60),
            "momus": CircuitBreaker("momus", failure_threshold=3, reset_timeout=60),
            "librarian": CircuitBreaker("librarian", failure_threshold=3, reset_timeout=60),
        }
        ...
        self.agent_limits = {
            "hephaestus": 2, "sisyphus junior": 4, "oracle": 1,
            "momus": 2, "librarian": 3,
        }
    ...
    def pre_execute(self, task, intent="UNKNOWN", complexity="MED", risk="LOW"):
        ...
        result = {"agent": agent, "plan": plan, "loop": loop, "context": context, "task": task}
...
# Circuit breaker tracks failure via call() wrapper
                    return {"status": "blocked", "reasons": verdict.reasons, "revisions": revision_count}
...
# Circuit breaker tracks success/failure via call() wrapper
    ...
    def _claims_to_dicts(self, claims):
        return [{"claim_type": c.claim_type, "support_status": c.support_status, "text": c.text} for c in claims]
    ...
    def get_pattern_stats(self):
        ...
        return {
            "cache": {"hits": self.cache_hits, "misses": self.cache_misses, "size": len(self.cache)},
            "intent": {"history_size": len(self.intent_history), "patterns": len(self.intent_patterns)},
            "bulkhead": {a: f"{self.agent_load[a]}/{self.agent_limits.get(a, 2)}" for a in self.agent_limits},
