...
class WorkingMemory:
    ...
    def to_context_string(self) -> str:
        ...
        if self.failure_counts:
            failures = ", ".join(f"{k}: {v} failures" for k, v in self.failure_counts.items())
...
class CircuitBreaker:
    ...
    def record_failure(self, tool_name: str) -> None:
        ...
        if state.failure_count > 0 and (now - state.first_failure_time) > self.time_window:
            state.failure_count = 0
        ...
        if state.failure_count >= self.failure_threshold:
            ...
            logger.warning(f"CircuitBreaker: {tool_name} OPEN after {state.failure_count} failures")
...
class ReActAgent:
    ...
    def __init__(
        ...
        self.model_classifier = model_classifier or CLASSIFIER
    ...
    async def run(
        ...
        for step_num in range(1, self.max_steps + 1):
            ...
            if action.tool_name and not self.circuit_breaker.is_allowed(action.tool_name):
                logger.warning(f"ReActAgent: Circuit breaker blocked {action.tool_name}")
            ...
            logger.info(f"ReActAgent: Action={action.tool_name}, Success={observation.success}")
            ...
            step = ReActStep(
                ...
                action=action,
                observation=observation,
                status=ReActStatus.ACTING if observation.success else ReActStatus.FAILED,
            )
            steps.append(step)
...
{history_text}"""
        ...
        user_message = (
            f"Step {step_num}: What should I do next to complete this task?\n\nTask: {task}"
        )
...
Provide a clear, concise answer based on what was learned."""
        ...
        try:
            ...
            return response.get("text", response.get("thought", "Unable to generate final answer"))
        except Exception as e:
    ...
    def _format_steps_history(self, steps: List[ReActStep]) -> str:
        ...
        for step in steps:
            ...
            if step.action:
                lines.append(f"  Action: {step.action.tool_name}({json.dumps(step.action.params)})")
