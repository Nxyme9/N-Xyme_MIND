...
# BrainBridge import with graceful fallback
try:
    from src.ui.tui.brain_bridge import BrainBridge

    BRAIN_BRIDGE_AVAILABLE = True
except ImportError:
    BRAIN_BRIDGE_AVAILABLE = False
    _bridge = None
...
@router.post("/execute")
async def execute_message(req: ExecuteRequest):
    if not BRAIN_BRIDGE_AVAILABLE:
        return {"error": "BrainBridge not available"}

    bridge = BrainBridge(model=req.model)

    if req.conversation_history:
        result = await bridge.process_with_context(
            req.message, req.conversation_history
        )
    else:
        result = await bridge.process_message(req.message)
...
async def route_task(req: RouteRequest):
    ...
    return {
        "target": plan.target,
        "use_evidence": plan.use_evidence,
        "use_critic": plan.use_critic,
        "loop_cap": plan.loop_cap,
        "reason": plan.reason,
    }
...
async def search_memory(req: MemorySearchRequest):
    if req.layer == "semantic":
        ...
        results = [
            i
            for i in _working_mem.get_all()
            if req.query.lower() in i.key.lower()
            or req.query.lower() in i.value.lower()
        ]
        return [
            {"key": i.key, "value": i.value, "activation": i.activation}
            for i in results[:5]
        ]
    return []
