"""Memory Consolidation — Sleep Engine.

Implements the 3-tier consolidation pipeline from MEMORY-CONSOLIDATION.md
and the bleeding edge memory plan.

Phase 1 — Journal (every session end):
  Group episodic memories by topic cluster → generate cluster summaries

Phase 2 — Consolidate (daemon, every 6h):
  Cross-cluster synthesis → merge overlapping summaries into principles

Phase 3 — Prune (daemon, daily):
  Mark source memories as archived after consolidation
...
- Mastra Observational Memory — 94.87% LongMemEval
"""
...
class SleepEngine:
    ...
    def journal_phase(self, hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            # Get recent memories grouped by kind
            cursor = conn.execute(
            ...
            for cluster_key, memories in clusters.items():
                ...
                # Generate summary (in production, use LLM; for now, concatenate)
            ...
            conn.execute(
                ...
                (
                    "journal",
                    len(rows),
                    0,  # No archiving in journal phase
                    datetime.now(timezone.utc).isoformat(),
                    datetime.now(timezone.utc).isoformat(),
                    elapsed,
                ),
    ...
    def consolidate_phase(self) -> dict[str, Any]:
        ...
        Finds summaries with high semantic overlap and merges them into
        higher-level abstractions (principles).
        ...
        try:
            ...
            # Find overlapping summaries (simple keyword overlap for now)
    ...
    def prune_phase(self, days_old: int = 7) -> dict[str, Any]:
        ...
        Marks source memories as archived if they've been consolidated
        and the consolidation is older than days_old.
        ...
        try:
            ...
            # Get consolidated memories older than cutoff
            cursor = conn.execute(
            ...
            for (source_ids_json,) in rows:
                ...
                for source_id in source_ids:
                    # Mark as archived (update tier to 'archived')
                    cursor = conn.execute(
    ...
    def get_consolidation_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            cursor = conn.execute(
                "SELECT SUM(memories_archived) FROM consolidation_log"
            )
