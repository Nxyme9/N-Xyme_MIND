...
class AdaptiveDecay:
    ...
    def compute_decay_score(
        ...
        temporal = math.exp(-age_days / max(1, stability * 10))  # Scale for slower decay
        ...
        if last_accessed:
            try:
                ...
                recency_score = max(0.0, 1.0 - days_since_access / 90.0)  # 90-day window
        ...
        # Weighted combination (FadeMem-style additive formula)
        overall = (
        ...
        try:
            ...
            for memory_id, score in rows:
                if score >= THRESHOLD_ACTIVE:
                    # Active: ensure tier is not archived
                    conn.execute(
