...
# Model capability matrix (pre-computed benchmarks) - REFINED SELECTION
MODEL_CAPABILITIES = {
    # Fast/light models for simple tasks
    "minimax-m2.5": {
        "reasoning": 0.85,
        "coding": 0.85,
        "creative": 0.82,
    ...
    "mimo-v2-omni": {
        ...
        "cost_per_1m": 0.0,
        "speed": "medium",
        "best_for": ["creative", "multimodal"],
    },
    # Reasoning/analysis powerhouse
    "deepseek-r1": {
        "reasoning": 0.93,
        "coding": 0.88,
        "creative": 0.82,
        ...
        "cost_per_1m": 0.0,
        "speed": "slow",
        "best_for": ["complex", "reasoning", "analysis", "math"],
    },
    ...
    "gemini-2.5-flash": {
        ...
        "cost_per_1m": 0.0,
        "speed": "fast",
        "best_for": ["summarization", "long_context"],
    },
    # NEW: Qwen3 for coding specifically
    "qwen3.6-plus": {
        "reasoning": 0.92,
        "coding": 0.95,
        "creative": 0.88,
...
class RouterBrain:
    ...
    def _analyze_with_keywords(
        ...
        # Agent-based hints - prefer minimax for all agents
        agent_hints = {
            "hephaestus": {"coding": 0.95},
            "oracle": {"analysis": 0.9, "reasoning": 0.9},
            "explore": {"analysis": 0.9},
            "librarian": {"summarization": 0.9, "analysis": 0.8},
            "sisyphus": {"reasoning": 0.9, "analysis": 0.8},
            "prometheus": {"reasoning": 0.9, "analysis": 0.8},
            "momus": {"analysis": 0.95, "reasoning": 0.9},
        }
        ...
        return {
            ...
            "best_score": round(best_model["score"], 3),
            "model_scores": {
                m["model"]: round(m["score"], 3)
                for m in sorted(model_scores, key=lambda m: m["score"], reverse=True)[
                    :3
                ]
            },
        }
...
Agent: {agent_type}""",
    ...
    def _estimate_complexity(self, prompt: str, system_prompt: str = "") -> str:
        ...
        if any(kw in text for kw in complex_kws):
            return "complex"
        if any(kw in text for kw in simple_kws):
            return "simple"
        return base
    ...
    def _get_required_capabilities(self, categories: List[str]) -> Dict[str, float]:
        ...
        weights = {
            "coding": {"coding": 0.9, "reasoning": 0.6},
            "reasoning": {"reasoning": 0.9, "analysis": 0.7},
            "creative": {"creative": 0.9},
            "math": {"math": 0.9, "reasoning": 0.6},
            "summarization": {"summarization": 0.9},
            "analysis": {"analysis": 0.9, "reasoning": 0.6},
        }
    ...
    def _score_models(
        self, required_caps: Dict[str, float], complexity: str
    ) -> List[dict]:
        scores = []
        complexity_bonus = {"simple": 0.15, "medium": 0.05, "complex": 0.15}.get(
            complexity, 0.0
        )

        # Get primary category for matching
        primary_cat = (
            max(required_caps.items(), key=lambda x: x[1])[0] if required_caps else None
        )
        ...
        for model_name, caps in MODEL_CAPABILITIES.items():
            ...
            # Speed penalty for simple tasks - prefer fast models
            ...
            # Bonus only for EXACT primary category match in best_for
            best_for = caps.get("best_for", [])
            if primary_cat and primary_cat in best_for:
                cap_score += 0.08

            # Context penalty
            context_penalty = -0.15 if caps.get("context_window", 0) < 8192 else 0.0

            scores.append(
                {
                    "model": model_name,
                    "score": max(0.0, cap_score + complexity_bonus + context_penalty),
                }
            )
        return scores
