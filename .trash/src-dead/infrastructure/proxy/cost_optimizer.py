...
MODEL_COSTS = {
    ...
    "deepseek-r1": {"input": 0.0, "output": 0.0, "provider": "openrouter"},
    "gemini-2.5-flash": {"input": 0.0, "output": 0.0, "provider": "google"},
}
...
class CostTracker:
    ...
    def get_model_stats(self, model: str) -> dict:
        with self._lock:
            ...
            total_cost = (
                u.get("total_input_tokens", 0) * cost["input"]
                + u.get("total_output_tokens", 0) * cost["output"]
            ) / 1_000_000
            return {
                "model": model,
                ...
                "success_rate": round(u.get("success_rate", 1.0), 3),
                "avg_latency_ms": round(u.get("avg_latency_ms", 0.0), 1),
                "total_tokens": u.get("total_input_tokens", 0)
                + u.get("total_output_tokens", 0),
            }
    ...
    def select_cheapest(self, min_quality: float = 0.0) -> Optional[str]:
        ...
        for model in MODEL_COSTS:
            ...
            score = (
                ...
                + (1.0 / (1.0 + stats["avg_latency_ms"] / 1000)) * 0.4
            )
