...
def validate_task_call(kwargs):
    ...
    if has_subagent and kwargs["subagent_type"] not in VALID_SUBAGENT_TYPES:
        errors.append(
            f"ERROR: Invalid subagent_type '{kwargs['subagent_type']}'. Valid: {VALID_SUBAGENT_TYPES}"
        )

    # Check 3: Valid category
    if has_category and kwargs["category"] not in VALID_CATEGORIES:
        errors.append(
            f"ERROR: Invalid category '{kwargs['category']}'. Valid: {VALID_CATEGORIES}"
        )
    ...
    if "run_in_background" in kwargs and not isinstance(
        kwargs["run_in_background"], bool
    ):
    ...
    if (
        ...
        or not kwargs["prompt"]
        or len(kwargs["prompt"].strip()) < 50
    ):
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            'Usage: python3 validate-agent-call.py \'{"subagent_type": "explore", ...}\''
        )
        sys.exit(1)
