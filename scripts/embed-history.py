...
HISTORY = os.path.expanduser("~/AppData/Roaming/Microsoft/Windows/PowerShell/PSReadLine/ConsoleHost_history.txt")
...
def add_episode(text, name):
    r = requests.post(GRAPHITI, json={
        ...
        "params": {"name": "graphiti_add_episode", "arguments": {"text": text, "name": name, "source": "command-history"}}
    })
...
lines = open(HISTORY, 'r', encoding='utf-8').readlines()
last = get_last_line()
...
for i in range(0, len(new_lines), batch_size):
    ...
    text = "Command history batch:\n" + "\n".join(f"  {j+1}. {cmd.strip()}" for j, cmd in enumerate(batch) if cmd.strip())
