...
import { useToast, Toaster } from '@/context/ToastContext';

interface DictateUIProps {
  className?: string;
}
...
export const DictateUI = ({ className }: DictateUIProps) => {
  ...
  const startRecording = useCallback(async () => {
    try {
        ...
        const pollInterval = setInterval(async () => {
          ...
          if (statusRes.ok) {
            ...
            if (status.last_result && status.last_result.text) {
              ...
              addToast("success", "Dictation complete!");
            }
          }
        }, 300);
      ...
      addToast("error", "Dictation service unavailable. Make sure nxyme-dictate is running.");
    }
  }, []);
  ...
  const resetDictation = useCallback(() => {
    ...
    }).catch(e => console.warn('Failed to stop dictation:', e));
