  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-2 max-w-md">
        <span className="text-xs text-muted-foreground mr-2">Quick starts:</span>
        {suggestions.map((suggestion, idx) => (
          <button
            key={idx}
            onClick={() => onCreate?.()}
            className="text-xs px-2 py-1 rounded-full bg-secondary/50 hover:bg-secondary text-secondary-foreground transition-colors"
          >
            {suggestion}
          </button>
        ))}
